package sg.activities.model
{
    ...
    public class ModelPayOnce extends ViewModelBase
    {
		public static const TYPE_DAYS:String = "days";	// 按开服时间开启的活动
		public static const TYPE_DATE:String = "date";	// 按实际时间开启的活动

		// 单例
		private static var sModel:ModelPayOnce = null;
		
		public static function get instance():ModelPayOnce
		{
			return sModel ||= new ModelPayOnce();
		}
		
        private var cfg:Object;
        private var _actId:Object = {};
        private var pay_ploy:Object;
        public function ModelPayOnce()
        {
            this.cfg = ConfigServer.ploy['pay_ploy']; // 获取配置
        ...
        override public function refreshData(data:*):void {
            ...
            for(var len:int = tempKeys.length - 1; len >= 0; len--)
            {
                ...
                if (!element) return;
                if (element.type === 'once' && this.getRemainingTime(id)) {
        ...
        public function getKeysById(id:String):Array {
            var cfg:Object = this.getConfigByID(id);
            var keys:Array = [];
            ModelManager.instance.modelUser.getPayList().forEach(function(obj:Object):void {
                cfg.reward[obj.cost_cfg] && keys.push(obj.cost_cfg);
            } ,this);
            return keys;
        }
        ...
        public function checkRewardByType(type:String):Boolean
        {
            ...
            var keys:Array = this.getKeysById(id);

            var len:int = keys.length;
            for(var i:int = 0; i < len; i++){
                var needMoney:int = parseInt(keys[i]);
                if (pay_list.indexOf(needMoney) !== -1 && reward_list.indexOf(needMoney) === -1) {
                    return true;
                }
            }
            return false;
        ...
        {
            // 获取配置
            var pay_ploy:Object = this.cfg = ConfigServer.ploy['pay_ploy'];
            var keys:Array = ObjectUtil.keys(pay_ploy);
            var len:int = keys.length;
            for(var i:int = 0; i < len; i++)
            {
                var element:Object = pay_ploy[keys[i]];
                if (element.type === 'once') {

                    // 检测活动时间
                    var active:Boolean = false;
                    if (element.days) {
                        var days:Array = element.days;
                        var currentDays:int = ModelManager.instance.modelUser.getGameDate();
                        currentDays >= days[0] && currentDays < (days[0] + days[1]) && (active = true);
                    }
                    else if (element.date) {
                        var startDate:Array = element.date[0];
                        var endDate:Array = element.date[1];
                        var startTime:Number = new Date(startDate[0], startDate[1], startDate[2]).getTime();
                        var endTime:Number = new Date(endDate[0], endDate[1], endDate[2]).getTime() + Tools.oneDayMilli;
                        var serverTime:Number = ConfigServer.getServerTimer();
                        serverTime > startTime && serverTime < endTime && (active = true);
                    }
                    // active && this.keys.push(keys[i]);
                }
            }
        }
