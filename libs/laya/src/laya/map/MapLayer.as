package laya.map {
	...
	public class MapLayer extends Sprite {
		...
		protected var _map:TiledMap;	///lichuang 修改
		public var _mapData:Array = null;
		
		protected var _tileWidthHalf:Number = 0;
		protected var _tileHeightHalf:Number = 0;
		
		private var _mapWidthHalf:Number = 0;		///lichuang 修改
		private var _mapHeightHalf:Number = 0;		///lichuang 修改
		 ...
		 * 当前需要更新的gridSprite列表 
		 */		
		protected var _showGridList:Array = [];		///lichuang 修改
		...
		/**lichuang修改**/  
		public function getPos(tileX:Number, tileY:Number, screenPos:Point = null):void {
			if (screenPos) {
				switch (_map.orientation) {
					case TiledMap.ORIENTATION_STAGGERED:
						tileX = Math.floor(tileX);
		...
		/**lichuang修改**/  
		public function getTileGrid(screenX:Number, screenY:Number, result:Point = null):Boolean {			
			var tTileW:int = _map.tileWidth;
			var tTileH:int = _map.tileHeight;
		...
		public function getDrawSprite(gridX:int, gridY:int):GridSprite {
			//lichuang修改 start
			var tSprite:GridSprite = this.createSprite();
			//lichuang修改 end
		...
		/**lichuang添加**/
		public function createSprite():GridSprite {
			...
			if (tileY >= 0 && tileY < _map.numRowsTile && tileX >= 0 && tileX < _map.numColumnsTile) {
				...
				if (tMapData != null && tIndex < tMapData.length) {
					if (tMapData[tIndex] != 0) {
						var ii:int = tMapData[tIndex];
						var sx:Number = 1;
						var sy:Number = 1;
						if (ii > 4000000) {
							var t:int = parseInt((ii / 1000000000).toString());
							if(t == 1) {
								sy = -1
							} else if (t == 2) {
								sx = -1;
						...
						var tTileTexSet:TileTexSet = _map.getTexture(ii);
						if (tTileTexSet) {
							var tX:Number = 0;
							var tY:Number = 0;
							...
							if (tTileTexSet.isAnimation) {
								...
								if (sx == 1 && sy == 1) {
									...
									var w:Number = sx == -1 ? tX + tTileTexSet.offX + this._map.tileWidth : tX + tTileTexSet.offX;
									var h:Number = sy == -1 ? tY + tTileTexSet.offY : tY + tTileTexSet.offY;
									gridSprite.graphics.drawTexture(tTileTexSet.texture, 0, 0, 0, 0, new Matrix(sx, 0, 0, 1, w, h));
								}
							}
							return true;
						}
