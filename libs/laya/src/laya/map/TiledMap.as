package laya.map {
	...
	import sg.map.utils.Math2;			//lichuang添加
	import sg.scene.constant.ConfigConstant;		//lichuang添加
	import sg.map.model.MapModel;		//lichuang添加
	...
	public class TiledMap {
		...
		public var _layerArray:Array = []; //这里保存所有的MapLayer对象   lichuang修改
		public var _renderLayerArray:Array = [];//这里保存需要渲染的MapLayer对象   lichuang修改
		private var _gridArray:Array = []; //保存所有的块数据
		//地图块相关的
		private var _showGridKey:Boolean = false; //是否显示块边界线（用来调试用）   lichuang修改
		private var _totalGridNum:int = 0; //一层中的GridSprite的总数
		private var _gridW:int = 0; //地图的横向块数
		private var _gridH:int = 0; //地图的坚向块数
		
		private var _gridWidth:Number = 450; //块的默认宽度  lichuang修改
		private var _gridHeight:Number = 450; //块的默认高度  lichuang修改
		...
		//lichuang修改 start
		public var _pivotScaleX:Number = 0.5;
		public var _pivotScaleY:Number = 0.5;
		...
		public function createMap(mapName:String, viewRect:Rectangle, completeHandler:Handler, viewRectPadding:Rectangle = null, gridSize:Point = null, enableLinear:Boolean = true, limitRange:Boolean = false):void {
			...
			this.onJsonComplete(mapName);		//lichuang修改
		}
		...
		private function onJsonComplete(e:*):void {
			...
			var tJsonData:* = _jsonData = Laya.loader.getRes(e);		//lichuang修改
			...
			//lichuang修改 start
			for (var k:int = 0, len:int = _tileSetArray.length; k < len; k++) {
				var tPath:String = mergePath(_resPath, _tileSetArray[k].image);
				this.onTextureComplete(_tileSetArray[k], Laya.loader.getRes(tPath));
			}
		 ...
		 * @param	e 纹理数据
		 */
		private function onTextureComplete(tTileSet:TileSet, tTexture:Texture):void {
			var json:* = _jsonData;
		...
		public function set scale(scale:Number):void {
			...
			_scale = scale;
			//lichuang修改 start
			var oldW:Number = _viewPortWidth;
			var oldH:Number = _viewPortHeight;
			//lichuang修改 end
			...
			//lichuang修改 start
			oldW = _viewPortWidth - oldW;
			oldH = _viewPortHeight - oldH;			
			_viewPortX -= oldW * _pivotScaleX;
			_viewPortY -= oldH * _pivotScaleY;
			//lichuang修改 end
		...
		public function moveViewPort(moveX:Number, moveY:Number):void {
			...
			//{
				...
				moveX = this.limitX(moveX);
				moveY = this.limitY(moveY);
		...
		/**lichuang添加**/
		public var limitRect:Rectangle = new Rectangle();
		/**lichuang修改**/
		public function limitX(moveX:Number):Number {
			return Math2.range(moveX, (this.width - this._viewPortWidth + (this.limitRect.width / this.scale)) , -this.limitRect.x / this.scale);
		}
		/**lichuang修改**/
		public function limitY(moveY:Number):Number {
			//this.limitRect.y = this.limitRect.height = 0;
			// return Math2.range(moveY, (this.height - (Browser.height) * (this._rect.width / Browser.width) / this._scale) , 0);
			return Math2.range(moveY, (this.height - (Laya.stage.height) * (this._rect.width / Laya.stage.width) / this._scale + (this.limitRect.height / this.scale)) , -this.limitRect.y / this.scale);
		}
		...
		{
			//lichuang修改 start
			//var viewPortX1 = this.limitX(this._viewPortX + dx);
			//var viewPortY1 = this.limitY(this._viewPortY + dy);
			//dx = viewPortX1 - this._viewPortX;
			//dy = viewPortY1 - this._viewPortY;
		...
		private function updateViewPort():void {
			this._viewPortX = this.limitX(this._viewPortX);
			this._viewPortY = this.limitY(this._viewPortY);
		...
		private function showGrid(gridX:int, gridY:int):void {
			...
			var i:int, j:int;
			var tGridSprite:GridSprite;		//lichuang修改
				...
				for (var z:int = 0; z < _layerArray.length; z++) {
					...
					if (!tGridSprite.isHaveAnimation) {
						...
						if (_showGridKey) {
							...
							sp.graphics.drawRect(0, 0, tGridWidth, tGridHeight, null, tColorStr);
							tGridSprite.addChild(sp);
						}
					if (!enableMergeLayer) {
						if (tGridSprite.drawImageNum > 0) {
							tLayer.addChildAt(tGridSprite, 0);
					...
					}else
					{
						...
						{
							tDrawMapLayer.addChildAt(tTGridSprite, 0);
