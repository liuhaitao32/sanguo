package laya.wx.mini {
	...
	public class MiniFileMgr{
		...
		/** lht 加 清理忽略的清单 */
		public static var ignoreList:Array;//lht
		/**
		 * @private 
		 * 是否是本地4M包文件 
		 * @param url
		...
		private static function onClearCacheRes():void
		{
			...
			var pushIn:Boolean = false;//是否进入队列
			var len:int=0;
			for(var key:String in filesListObj)
			{
				pushIn = true;
				if(MiniFileMgr.ignoreList){
					//检查是否忽略
					len = MiniFileMgr.ignoreList.length;
					for(var j:int = 0; j < len; j++)
					{
						if(key.indexOf(MiniFileMgr.ignoreList[j])>-1){
							pushIn = false;
							break;
						}
					}
		...
		public static function existDir(dirPath:String, callBack:Handler):void {
			var fileArr:* = null;
			try{
				fileArr = fs.readdirSync(dirPath+"/");
				//目录下的所有文件
			}catch(e:*){
				fileArr = null;
			}
			try{
				if(fileArr!=null){
					//如果存在就删除掉
					var len:int = fileArr.length;
					var fp:String = "";
					for(var i:int = 0; i < len; i++)
					{
						fp = getFileNativePath(fileArr[i]);
						fs.unlinkSync(fp);
					}
				}
			}catch(e:*){
