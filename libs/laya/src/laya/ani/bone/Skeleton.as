				...
				if (tEventData.time >= _player.playStart && tEventData.time <= _player.playEnd) {
					if (_player.currentPlayTime >= tEventData.time) {
						...
						if (_playAudio && tEventData.audioValue && tEventData.audioValue !== "null") {
							_soundChannel = SoundManager.playSound((_player.templet as Templet)._path + tEventData.audioValue, 1, Handler.create(this, _onAniSoundStoped));
					...
					_eventIndex++;
					_soundChannel = SoundManager.playSound((_player.templet as Templet)._path + tEventData.audioValue, 1, Handler.create(this, _onAniSoundStoped), null,  (_player.currentPlayTime - tEventData.time) / 1000);
					SoundManager.playbackRate = _player.playbackRate;
					_soundChannel && _soundChannelArr.push(_soundChannel);
				} else {
					_eventIndex++;
		...
		public function resume():void {
			...
			if (_pause) {
				...
				if (_soundChannelArr.length > 0) { // 有正在播放的声音
					...
					for (var len:Number = _soundChannelArr.length, i:Number =0; i < len; i++) {
						_soundChannel = _soundChannelArr[i];
						if (_soundChannel is WebAudioSoundChannel && (_soundChannel as WebAudioSoundChannel).audioBuffer) { // Thor 修改
