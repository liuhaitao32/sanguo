package laya.utils {
	...
	public class Timer {
		...
		//[IF-JS] private var _map:Object = {};
		/**@private */
		private var _laters:Array = [];
		/**@private */
		public var _handlers:Array = [];//lht 改
		/**@private */
		...
		public function _update():void {
					...
					if (t >= handler.exeTime) {
						if (handler.repeat) {
							if (!handler.jumpFrame) {
								...
								//lichuang 卡顿的 派发下 卡顿多少帧。 时间太长对应的类要进行回复。
								if (handler.caller is EventDispatcher) {
									if(timeCount > 3) handler.caller.event(EventConstant.REPEAT, timeCount);
								}
							}
						} else {
							handler.run(true);
			...
			for (var i:int = 0, n:int = laters.length - 1; i <= n; i++) {
				...
				if (handler.method !== null) {
					...
					//[IF-SCRIPT]delete _map[handler.key];
					handler.run(false);
				}
		...
		private function _recoverHandler(handler:TimerHandler):void {
			...
			//[IF-SCRIPT]if(_map[handler.key]==handler) delete _map[handler.key];
			handler.clear();
			_pool.push(handler);
		}
		...
		private function _indexHandler(handler:TimerHandler):void {
			...
			var mid:int = method.$_TID || (method.$_TID = (_mid++));
		 ...
		 * lht 改
		 */
		public function clearAll(caller:*,all:Boolean = false):void {
			if (!caller && !all) return;
			for (var i:int = 0, n:int = _handlers.length; i < n; i++) {
				var handler:TimerHandler = _handlers[i];
				if (handler.caller === caller || all) {
					//[IF-JS] delete _map[handler.key];
					/*[IF-FLASH]*/
					_map[handler.method] = null;
					handler.clear();
