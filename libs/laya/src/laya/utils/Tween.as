package laya.utils {
	...
	public class Tween {
		...
		public function set scale(value:Number):void {	
			this._startTimer = Browser.now();
			this._duration -= this._usedTimer;
			this._usedTimer = 0;
			this._duration *= value;
			
			for (var i:int = 0, len:int = this._props.length; i < len; i++) {
				var p:Number = this._props[i][1] + this._props[i][2];
				this._props[i][1] = this._target[this._props[i][0]];
				this._props[i][2] = p - this._props[i][1];
			}
		...
		public function set progress(v:Number):void {
			...
			this._startTimer = Browser.now() - this._delay - uTime;
				...
				/**lichuang添加**/
		public function get usedTimer():int {
			return _usedTimer;
		}
		/**lichuang添加**/
		...
		public function complete():void {
			if (!this._target) {
				return;
			}else if (this._target.destroyed){	//zhuda添加
				this.clear();
				return;
			}
		 ...
		 * @param	target 目标对象。
		 */
		public static function completeAll(target:Object):void {
			/*[IF-FLASH]*/
			if (!target) return;
			//[IF-JS]if (!target || !target.$_GID) return;
			/*[IF-FLASH]*/
