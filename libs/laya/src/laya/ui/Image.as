package laya.ui {
	...
	public class Image extends Component {
			...
			if (_skin != value) {
				...
				if (value) {
					...
					if (source) {
						...
						onCompResize();
						//如果曾经缓存过这个！ 直接给它加侦听事件。
						if (cacheImages[this._skin]) {
							this.cacheImage();
						}
					} else {
						Laya.loader.load(_skin, Handler.create(this, setSource, [_skin]), null, Loader.IMAGE, 1, true, _group);
						this.cacheImage();
					}
				} else {
					this.source = null;
		...
		private function cacheImage():void {
			cacheImages[this._skin] ||= [];
			if ( -1 == cacheImages[this._skin].indexOf(this)) {
				cacheImages[this._skin].push(this);
			}
			
			this.on(Event.DISPLAY, this, onDisplayHandler);
		}
		
		private function onDisplayHandler():void {
			//if (this.source == null) return;
			
			var skin:String = this._skin;
			this._skin = null;
			this.skin = skin;//显示的时候重新刷一遍。
		}
		
		public static function clearAllCache():void {
			for (var name:String in cacheImages) {
				var arr:Array = cacheImages[name];
				if (name.indexOf("hero") == 0 || name.indexOf("icon") == 0 || name.indexOf("science") == 0) {
					Loader.clearTextureRes(name);
				} else {
					Loader.clearRes(name);
				}
				trace("清理" + name);
			}
			cacheImages = {};
		...
		public static var cacheImages:Object = {};
